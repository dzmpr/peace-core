...
def phrase_builder(context: PhraseSubclass, phrase_class: PhraseClass, temp_phrase: List[Token], phrase_line: int) -> Phrase:
...
def build_block(phrase: Phrase, context: PhraseSubclass, temp_phrase: List[Token], phrase_line: int):
    ...
    if phrase.keyword.value == "main":
        if context == PhraseSubclass.program:
            ...
            raise PhraseBuildError(f"Phrase build error at line {phrase_line}.\n"
                                   f"Main block can not be defined inside {context.name}.", phrase_line)
...
def build_operator(phrase: Phrase, context: PhraseSubclass, temp_phrase: List[Token], phrase_line):
    if context != PhraseSubclass.program:
        if temp_phrase[0].token_class == TokenClass.word:
            phrase.keyword = temp_phrase[0]
        else:
...
def build_label(phrase: Phrase, context: PhraseSubclass, temp_phrase: List[Token], phrase_line):
    if context != PhraseSubclass.program:
        if temp_phrase[0].token_class == TokenClass.word:
            phrase.keyword = temp_phrase[0]
        else:
